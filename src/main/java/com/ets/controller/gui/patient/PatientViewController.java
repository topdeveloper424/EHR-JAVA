...
public class PatientViewController implements Initializable {
	...
	@FXML
	private Button selectPrivatePractice;
	...
	public void viewPatient() {
		 ...
		 * patientMasterData = new
		 * PatientEntityController().view(Global.clinic.getId());
		 * 
		 * }
		 */
	...
	public void viewCurrentCompany() {
		...
		// TableColumn<ObservableList<Patient>, String> Status = new
		// TableColumn<>("Concatentated Values");
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		addButton.setOnAction((event) -> {
			try {
				...
				PatientInputController patientInputController = (PatientInputController) new FXFormCommonUtilities()
		...
		viewPatientBtn.setOnAction((event) -> {
			try {
				...
				/*
				 * PatientEditController patientEditController = (PatientEditController) new
		...
		selectPrivatePractice.setOnAction((event) -> {
			...
			try {
				...
				if (selectedPatient != null) {
					...
					if (SearchPatientController.button == null) {
						...
						patientAppointmentInputController = (PatientAppointmentInputController) new FXFormCommonUtilities()
								.displayForm(formName, formTitle);
						...
						if (SearchPatientController.button.getId().equals("scheduleNewAppointmentBtn")) {
							...
							if (selectedPatient.getPcp() != null) {
								patientAppointmentInputController.setProviderName(selectedPatient.getPcp().getSuffix()
										+ " " + selectedPatient.getPcp().getFirstName() + " "
										+ selectedPatient.getPcp().getMiddleName() + " "
										+ selectedPatient.getPcp().getLastName());
							...
							String formTitle = formPath.context.getMessage("Title.ViewPatientAppointment", null,
									Locale.US);
							formPath.closeApplicationContext();

							patientAppointmentList = new PatientAppointmentEntityController()
									.viewById(selectedPatient.getId());
							PatientAppointmentViewController patientAppointmentViewController = (PatientAppointmentViewController) new FXFormCommonUtilities()
									.displayForm(formName, formTitle);
							patientAppointmentViewController.setPatientViewController(this,
									SearchPatientController.button);
							...
							VisitLogSelectController visitLogInputController = (VisitLogSelectController) new FXFormCommonUtilities()
									.displayForm(formName, formTitle);
							...
							// String formName = formPath.context.getMessage("VisitByPatient", null,
							// Locale.US);
							String formName = "/view/code/appointmentSetup/patient/VisitByPatient.fxml";
							// String formTitle1 = formPath.context.getMessage("VisitByPatient", null,
							// Locale.US);
							 ...
							 * 
							 * inventoryOutGoInputController.setPatient(selectedPatient.getPatientName().
							 * getFirstName());
							 * 
							 * }
							 */
