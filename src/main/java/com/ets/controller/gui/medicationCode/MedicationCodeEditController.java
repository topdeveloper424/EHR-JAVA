...
public class MedicationCodeEditController implements Initializable {
	...
	public void setMedication(Medication medication) {
		...
		if (medication.getMedicationClass() != null) {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		medicationClassButton.setOnAction((event) -> {
			try {
				...
				medicationClassViewController.setMedicationCodeEditController(this);
		...
		okButton.setOnAction((event) -> {
			try {
				...
				if (medicationClassTextField.getText().length() > 0) {
					MedicationClass medicationClass = new MedicationClassEntityController()
							.getByName(medicationClassTextField.getText());
